...
class TrajectorySummary(BaseModel):
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> 'TrajectorySummary':
        return cls(**data)
...
class CloudMessage(BaseModel):
    ...
    @classmethod
    def create_trajectory_upload(cls, trajectory_data: Trajectory) -> 'CloudMessage':
        """创建轨迹上传消息"""
        import time
        return cls(
            message_type=MessageType.AGENT_OUTPUT,  # 使用兼容的消息类型
            data={
            ...
            source="edge",
            destination="cloud",
            request_id=f"upload_{int(time.time())}",
            timestamp=str(time.time())
        )

    @classmethod
    def create_summary_request(cls, trajectory_id: str, request_summary: bool = True, request_evaluation: bool = True) -> 'CloudMessage':
        """创建总结请求消息"""
        import time
        return cls(
            message_type=MessageType.AGENT_OUTPUT,  # 使用兼容的消息类型
