...
@registry.register_task("neural")
class EvalNeural(BaseTask):
    ...
    def __init__(self,
        ...
        self.llm = llm  # FIXME currently agent llm = environment 
        #################################################################
        
        self.label_path = env_config.get("label_path", None)
        self.env_configs = self.load_annotation(self.label_path)
        __ = [_.update({'llm':llm}) for _ in self.env_configs]
        ...
        self.agentboard = TaskLogger(task_name="neural", log_path=log_path, max_num_steps=self.max_num_steps, baseline_dir=self.baseline_dir)
    ...
    def evaluate_env(self, id):
        ...
        env = load_environment("neural", self.env_configs[id])
        task_name = self.env_configs[id]['id']
        init_obs = env._get_obs()
        goal = env._get_goal()
        self.agent_config.update({'instruction': game_config['description']})
        self.agent = load_agent(self.agent_name, self.agent_config, self.llm)
        ...
        env_details = {"goal": self.agent.goal, "task_name": task_name,}
    ...
    def from_config(cls, 
        ...
        env_name = env_config.get("name", "neural")
        assert env_name == "neural"
